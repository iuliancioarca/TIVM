...
#	# Draw Get meas button	
	...
	CImGui.Button("Refresh measurements") && @async begin
		scope_conf.Measurement_Value1 = string(get_meas_data(scope, "MEAS1"))
		scope_conf.Measurement_Value2 = string(get_meas_data(scope, "MEAS2"))
		scope_conf.Measurement_Value3 = string(get_meas_data(scope, "MEAS3"))
		scope_conf.Measurement_Value4 = string(get_meas_data(scope, "MEAS4"))
		scope_conf.Measurement_Value5 = string(get_meas_data(scope, "MEAS5"))		
	end
#	# Draw Refresg settings button	
	CImGui.SameLine()
	CImGui.Button("Refresh settings") && @async begin
	 scope_conf.CH1_Volt_div = string(get_vertical_scale(scope, "CH1"))
     scope_conf.CH2_Volt_div = string(get_vertical_scale(scope, "CH2"))
     scope_conf.CH1_Offset = string(get_ch_position(scope, "CH1"))
     scope_conf.CH2_Offset = string(get_ch_position(scope, "CH2"))
     scope_conf.Time_div = string(get_horizontal_scale(scope, "Time"))
