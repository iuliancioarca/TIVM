...
function update_scope_conf!(scope_conf, scope, refresh_cnt, base)	 
	 ...
	 #(refresh_cnt==base*5) && ((scope_conf.t, scope_conf.y1) = fetch_wfm(scope, "CH1"))
	 (scope_conf.t, scope_conf.y1) = fetch_wfm(scope, "CH1")
	 
	 # conf acq chan2 and acquire	 
	 (refresh_cnt==base*10) && (conf_acq_ch(scope, "CH2"))
	 #(refresh_cnt==base*15) && ((scope_conf.t, scope_conf.y2) = fetch_wfm(scope, "CH2"))	 
	 (scope_conf.t, scope_conf.y2) = fetch_wfm(scope, "CH2")
